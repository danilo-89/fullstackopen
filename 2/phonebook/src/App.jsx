...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if(newName.trim()) {
      ...
      const isAlreadyThere = persons.filter((person) => person.name === newName)
      ...
      if (isAlreadyThere.length) {
        ...
        if (window.confirm(newName + " is already in the phonebook, replace old number with the new one?")) {
          mainService.update(isAlreadyThere[0].id, personObj)
            .then(result => {
              console.log(result)
              const personsUpdated = persons.map((person) => person.id === isAlreadyThere[0].id ? { ...personObj, id: person.id } : person)
            ...
            .catch((error) => {
              console.log(error)
              setStatusMessage(["Information of " + newName + " has already been removed from server", "error"])
              const personsUpdated = persons.filter((person) => person.id !== isAlreadyThere[0].id)
              setPersons(personsUpdated);
              setTimeout(() => {
                setStatusMessage([null, null])
              }, 4000)   
